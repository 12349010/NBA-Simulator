...
# ---------- session state ----------
for k in ["home_team","away_team","home_starters","home_bench",
...
if st.button("▶️ Simulate"):
    if "— Select —" in (st.session_state.home_team, st.session_state.away_team):
        st.error("Pick both teams first.")
        st.stop()
    ...
    def to_int(df:pd.DataFrame)->pd.DataFrame:
        ...
        out[nums]=out[nums].astype(int)
        return out

    ch,ca = st.columns(2)
    with ch:
        st.subheader(f"{cfg['home_team']} Box (avg)")
        st.dataframe(to_int(box_h),use_container_width=True)
    with ca:
        st.subheader(f"{cfg['away_team']} Box (avg)")
        st.dataframe(to_int(box_a),use_container_width=True)

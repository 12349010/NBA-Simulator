...
Pulls NBA rosters from Basketball‑Reference with automatic fallbacks:
• Tries current season, then walks back 5 seasons.
• If primary abbreviation fails, retries known alternates (e.g. BKN⇄BRK).
...
All results cached (12 h) in roster_cache.json.
"""
...
# Alternate / historic codes for fallback
...
def _fix(txt: str) -> str:
    if "Ã" in txt or "Å" in txt:           # charset garble fixer
        try:
...
def get_roster(team: str) -> Dict[str, List[str]]:
    """
    Returns {"starters":[5 players], "bench":[rest]}.
    • Checks current season then walks back 5 years.
    • Retries with alternate abbreviations if needed.
    """
    season = _season_year()
    ...
    def _attempt(abr: str, yr: int) -> Dict[str, List[str]] | None:
        ...
        if key in cache:
            return cache[key]
        try:
            tbl = _team_html_abbr(abr, yr).select_one("#roster")
            if tbl is None:
                return None
    ...
    # pass 1: primary abbr, seasons (season → season‑5)
    for off in range(6):
        roster = _attempt(primary, season - off)
        if roster:
            return roster
    ...
    for alt in ALT_ABR.get(primary, []):
        for off in range(6):
            roster = _attempt(alt, season - off)
            if roster:
                return roster

    raise ValueError(f"Unable to locate roster table for {team} "
                     f"(tried {primary} + alternates over 6 seasons)")

...
Loads all CSV tables under data/, normalizes columns, and exposes API functions:
 ...
 - get_common_roster(team_id, season)
Also provides internal DataFrames: _team_df, _game_df, _line_score_df,
_common_player_info_df, _inactive_players_df, _pbp_df
"""
import os
import glob
import pandas as pd
...
# -------------------------------------------------------------------
# NEW helper for season‐filtered roster dropdowns in app.py:
def get_common_roster(team_id, season):
    """
    Return only the common-player-info roster for a single team & season,
    filtering strictly by from_year ≤ season ≤ to_year.
    """
    return _common_player_info_df[
